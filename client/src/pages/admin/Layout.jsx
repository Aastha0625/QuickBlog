...
const Layout = () => {
    ...
    return (
        <>
            <div className='flex items-center justify-between py-2 h-[75px] px-4 sm:px-12 border-b border-gray-300'>
                <img src={assets.logo} alt="" className='w-32 sm:-w-40 cursor-pointer' 
                onClick={() => navigate('/')} />
                <button 
                    className='flex items-center gap-2 rounded-full bg-primary text-white text-sm cursor-pointer px-10 py-2.5' 
                    onClick={handleLogout} // Use the corrected logout function
                >
                    Logout
                </button>
            </div>
